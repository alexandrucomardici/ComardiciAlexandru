...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if($password !== $confirm){
        ...
        $message = "Parola trebuie sa aibă minim 6 caractere!";
    } else {
        // Hash parola
        $hash = password_hash($password, PASSWORD_DEFAULT);

        // pregătim insert
        $stmt = $conn->prepare("INSERT INTO utilizatori (nume, email, parola, telefon, adresa) VALUES (?, ?, ?, ?, ?)");
        if(!$stmt){
            die("Eroare la pregătirea statement-ului: " . $conn->error);
        }
        $stmt->bind_param("sssss", $fullname, $email, $hash, $telefon, $adresa);
        ...
        if($stmt->execute()){
            ...
            if($conn->errno === 1062){
                ...
                $message = "Eroare la crearea contului: " . $conn->error;
            }
        }
        $stmt->close();
    }
...
<div class="register-container">
    <div class="register-card">
        ...
        <form method="post" novalidate>
            ...
            <div class="mb-3">
                <label for="telefon" class="form-label">Număr de telefon</label>
                <input type="text" id="telefon" name="telefon" class="form-control" value="<?=htmlspecialchars($_POST['telefon'] ?? '')?>" required>
            </div>

            <div class="mb-3">
                <label for="adresa" class="form-label">Adresă</label>
                <input type="text" id="adresa" name="adresa" class="form-control" value="<?=htmlspecialchars($_POST['adresa'] ?? '')?>" required>
            </div>
